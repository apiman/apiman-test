...
public class HttpSecurityClientPolicyIT extends AbstractHttpSecurityPolicyIT{
    ...
    @ClientVersion(client = "client", contracts = @Contract(vPlan = "plan", vApi = "endpoint"),
        policies = @Policies(value = "plugins/httpsecurity/http_security",
        params = {"enabled", "false", "csp.mode", "DISABLED", "fo", "DENY", "xss", "ON", "cto", "true"}))
    @ApiKey(vPlan = "plan", vApi = "endpoint")
    private static String apikey;

    //Endpoint for another configuration
    @ApiVersion(api = "api", vPlans = {"configPlan"}, version = "config")
    @ManagedEndpoint
    private static String configEndpoint;

    @Plan(organization = "organization")
    @PlanVersion
    private static PlanBean configPlan;

    @Client(organization = "organization")
    private static ClientBean configClient;

    @ClientVersion(client = "configClient", contracts = @Contract(vPlan = "configPlan", vApi = "configEndpoint"),
            policies = @Policies(value = "plugins/httpsecurity/http_security",
                    params = {"enabled", "false", "csp.mode", "DISABLED", "fo", "DENY", "xss", "DISABLED", "cto", "true"}))
    @ApiKey(vPlan = "configPlan", vApi = "configEndpoint")
    private static String configApikey;
    ...
    protected String getConfigResourceURL(){
        return addApiKeyParameter(configEndpoint, configApikey);
    }
