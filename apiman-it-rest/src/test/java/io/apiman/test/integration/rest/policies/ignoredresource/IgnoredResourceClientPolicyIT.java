...
public class IgnoredResourceClientPolicyIT extends AbstractIgnoredResourcePolicyIT {

    @Api(organization = "organization")
    @ApiVersion(vPlans = "plan")
    @ManagedEndpoint private static String endpoint;

    @Plan(organization = "organization")
    @PlanVersion private static PlanBean plan;

    @Client(organization = "organization")
    @ClientVersion(policies = @Policies("ignored_001"),  contracts = @Contract(vPlan = "plan", vApi = "endpoint"))
    @ApiKey(vApi = "endpoint", vPlan = "plan")
    private static String apikey;

    @Override protected String getResourceURL(String path) {
