...
public class URLRewritingStringHeaderAndBodyIT extends AbstractURLRewritingPolicyIT{
    ...
    @ApiVersion(api = "api", endpoint = @Endpoint(value = ENDPOINT_DATA),
        policies = @Policies(value = "url_rewrite_001",
            params = {"regex", STRING_VALUE_UNCHANGED, "replace", STRING_VALUE_CHANGED, "header", "true", "body", "true" }))
    ...
    public void shouldPassWhenRewritingHeaderAndBodyIsSuccessful(){
        ...
        Assert.assertTrue(originValue.contains(STRING_VALUE_UNCHANGED));
        Assert.assertTrue(rewriteValue.contains(STRING_VALUE_CHANGED));
