 ...
 * @author jrumanov
 */
public class ClientUnregisterIT extends AbstractClientUITest {
    ...
    public static void setUp() {
        ...
        expectedStatus = new ResponseSpecBuilder()
            .expectBody("status", equalTo("Retired"))
            .build();
    }
    ...
    public void canUnregisterClient() {
        ClientDetailPage clientDetailPage =
            open(ClientDetailPage.class, organization.getId(), client.getId())
                .unregister()
                .confirmDialog();

        clientDetailPage.unregisterButton().shouldNotBe(visible);
        clientDetailPage.registerButton().shouldNotBe(visible);
        clientDetailPage.status().shouldHave(text("Retired"));
